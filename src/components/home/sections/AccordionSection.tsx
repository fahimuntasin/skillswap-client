  ...
  return (
    <section className="py-24">
      <div className="mx-auto grid max-w-[1280px] gap-20 px-6 lg:grid-cols-[45%_55%]">
        <div>
          <h2 className="text-[36px] font-bold text-[#0F172A] dark:text-[#f8fafc] dark:text-[#f8fafc] tracking-[-0.02em] mb-2">How SkillSwap works</h2>
          <p className="text-[16px] text-[#64748B] dark:text-[#94a3b8] dark:text-[#94a3b8] mb-12">Three simple steps to get your tasks done</p>

          <div>
            {items.map((item, i) => (
              <div key={item.title} className="border-t border-[#F1F5F9] dark:border-[#2a2a3e]">
                <button
                  onClick={() => setOpen(open === i ? -1 : i)}
                  className="flex w-full items-center justify-between py-5 text-left"
                >
                  <span className="text-[17px] font-medium text-[#0F172A] dark:text-[#f8fafc] dark:text-[#f8fafc]">{item.title}</span>
                  <ChevronDownIcon className={`size-5 text-[#94A3B8] dark:text-[#64748b] transition-transform ${open === i ? "rotate-180" : ""}`} />
                </button>
                {open === i && (
                  <div className="pb-5 -mt-1">
                    <p className="text-[15px] leading-[1.7] text-[#64748B] dark:text-[#94a3b8] dark:text-[#94a3b8] mb-3">{item.desc}</p>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium text-[#0F172A] dark:text-[#f8fafc] dark:text-[#f8fafc]">Sarah Chen</span>
              </div>
              <span className="text-xs px-2 py-1 rounded-full bg-[#DCFCE7] text-[#166534] font-medium">Open</span>
            </div>
            <h3 className="text-[17px] font-semibold text-[#0F172A] dark:text-[#f8fafc] dark:text-[#f8fafc]">Design a modern landing page</h3>
            <p className="text-sm text-[#64748B] dark:text-[#94a3b8] dark:text-[#94a3b8]">Fixed-price · $150 · Due Jul 15</p>
            <div className="flex -space-x-2 pt-2">
              ...
              <span className="text-xs text-[#64748B] dark:text-[#94a3b8] dark:text-[#94a3b8] pl-4 pt-1">3 proposals</span>
            </div>
          </div>
        </div>
      </div>
