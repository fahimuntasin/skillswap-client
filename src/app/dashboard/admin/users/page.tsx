...
export default function AdminUsersPage() {
  ...
  return (
    <DashboardShell role="admin" userName="Admin">
      ...
      <div className="rounded-xl border border-[#F1F5F9] dark:border-[#2a2a3e] dark:border-[#2a2a3e] bg-white dark:bg-[#1c1a3a] dark:bg-[#1c1a3a]">
        <div className="grid grid-cols-[1fr_1fr_80px_80px_120px] gap-4 border-b border-[#F1F5F9] dark:border-[#2a2a3e] dark:border-[#2a2a3e] px-6 py-3 text-xs font-semibold uppercase text-[#94A3B8] dark:text-[#64748b]">
          <span>Name</span><span>Email</span><span>Role</span><span>Status</span><span>Actions</span>
        </div>
        {users.map((u: any) => (
          <div key={u._id} className="grid grid-cols-[1fr_1fr_80px_80px_120px] gap-4 items-center px-6 py-4 border-b border-[#F1F5F9] dark:border-[#2a2a3e] dark:border-[#2a2a3e] last:border-0 text-sm">
            <div className="flex items-center gap-2">
              <span className="font-medium text-[#0F172A] dark:text-[#f8fafc] dark:text-[#f8fafc]">{u.name}</span>
              {u.verified && <ShieldCheckIcon className="size-4 text-[#7C3AED]" />}
            </div>
            <span className="text-[#64748B] dark:text-[#94a3b8] dark:text-[#94a3b8]">{u.email}</span>
