...
export default function AdminUsersPage() {
  return (
    <DashboardShell role="admin" userName="Admin">
      <div className="mb-6"><p className="text-sm font-semibold text-[#7C3AED] mb-1">Admin</p><h1 className="text-[28px] font-bold text-[#0F172A]">Manage Users</h1></div>
      <div className="rounded-xl border border-[#F1F5F9] bg-white">
        <div className="grid grid-cols-[1fr_1fr_100px_100px_100px] gap-4 border-b border-[#F1F5F9] px-6 py-3 text-xs font-semibold uppercase text-[#94A3B8]">
          <span>Name</span><span>Email</span><span>Role</span><span>Status</span><span>Action</span>
        </div>
        {users.map((u) => (
          <div key={u.id} className="grid grid-cols-[1fr_1fr_100px_100px_100px] gap-4 items-center px-6 py-4 border-b border-[#F1F5F9] last:border-0 text-sm">
            <span className="font-medium text-[#0F172A]">{u.name}</span><span className="text-[#64748B]">{u.email}</span>
