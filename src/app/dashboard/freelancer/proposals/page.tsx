  ...
  return (
    <DashboardShell role="freelancer" userName="Sarah">
      <div className="mb-6">
        ...
        <h1 className="text-[28px] font-bold text-[#0F172A]">My Proposals</h1>
      </div>
      <div className="rounded-xl border border-[#F1F5F9] bg-white">
        <div className="grid grid-cols-[1fr_100px_120px_100px] gap-4 border-b border-[#F1F5F9] px-6 py-3 text-xs font-semibold uppercase text-[#94A3B8]">
          <span>Task</span><span>Bid</span><span>Date</span><span>Status</span>
        </div>
        {proposals.map((p) => (
          <div key={p.id} className="grid grid-cols-[1fr_100px_120px_100px] gap-4 items-center px-6 py-4 border-b border-[#F1F5F9] last:border-0 text-sm">
            <span className="font-medium text-[#0F172A]">{p.task}</span>
            <span className="font-semibold">${p.budget}</span>
            <span className="text-[#64748B]">{new Date(p.date).toLocaleDateString()}</span>
