  ...
  return (
    <DashboardShell role="freelancer" userName="Sarah">
      <div className="mb-6">
        ...
        <h1 className="text-[28px] font-bold text-[#0F172A] dark:text-[#f8fafc] dark:text-[#f8fafc]">Active Projects</h1>
      </div>
        ...
        {projects.map((p) => (
          <div key={p.id} className="rounded-xl border border-[#F1F5F9] dark:border-[#2a2a3e] bg-white dark:bg-[#1a1a2e] dark:bg-[#1a1a2e] p-5">
            <div className="flex items-start justify-between mb-3">
              <div>
                <h3 className="text-[15px] font-semibold text-[#0F172A] dark:text-[#f8fafc] dark:text-[#f8fafc]">{p.task}</h3>
                <p className="text-sm text-[#64748B] dark:text-[#94a3b8] dark:text-[#94a3b8]">Client: {p.client} · ${p.budget}</p>
              </div>
