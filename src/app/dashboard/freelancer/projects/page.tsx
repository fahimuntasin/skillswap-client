  ...
  return (
    <DashboardShell role="freelancer" userName="Sarah">
      <div className="mb-6">
        ...
        <h1 className="text-[28px] font-bold text-[#0F172A]">Active Projects</h1>
      </div>
        ...
        {projects.map((p) => (
          <div key={p.id} className="rounded-xl border border-[#F1F5F9] bg-white p-5">
            <div className="flex items-start justify-between mb-3">
              <div>
                <h3 className="text-[15px] font-semibold text-[#0F172A]">{p.task}</h3>
                <p className="text-sm text-[#64748B]">Client: {p.client} · ${p.budget}</p>
              </div>
