...
export default function PostTaskPage() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!session?.user?.email) {
      notifyError("Sign in required", "You must be signed in to post a task.")
      setLoading(false)
      return
    }
    try {
      ...
      setSuccess(true)
      notifySuccess("Task posted!", "Freelancers can now send you proposals.")
      setTimeout(() => router.push("/dashboard/client"), 1200)
    } catch {
      notifyError("Could not post task", "Please check your details and try again.")
    } finally {
      setLoading(false)
    }
  }
      ...
      <div className="max-w-[640px]">
        ...
        {success ? (
          <SuccessPanel
            ...
            description="Your task is live. Freelancers can browse it and send proposals."
            actionLabel="Go to dashboard"
            actionHref="/dashboard/client"
            secondaryLabel="Post another task"
            secondaryHref="/dashboard/client/post-task"
          />
        ) : (
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <PrimaryActionButton loading={loading} disabled={loading}>
              {loading ? "Posting your task..." : "Post task"}
            </PrimaryActionButton>
          </form>
        )}
      </div>
