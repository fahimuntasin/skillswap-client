...
export default function PaymentSuccessPage() {
  ...
  useEffect(() => {
    ...
    api.confirmPayment(sessionId).then(async (payment: any) => {
      let taskTitle = ""
      let freelancerName = payment.freelancer_email
      try {
        const task = await api.getTask(payment.task_id)
        taskTitle = task?.title || ""
      } catch {}
      try {
        const freelancers = await api.getFreelancers(100)
        const match = freelancers.find((f: any) => f.email === payment.freelancer_email)
        if (match?.name) freelancerName = match.name
      } catch {}
      setData({ ...payment, taskTitle, freelancerName })
          ...
          <>
            ...
            <div className="rounded-xl border border-[#F1F5F9] dark:border-[#2a2a3e] bg-white dark:bg-[#1c1a3a] p-5 text-left mb-8">
              <div className="grid gap-3 text-sm">
                <div className="flex justify-between gap-4"><span className="text-[#64748B]">Task</span><span className="font-semibold text-[#0F172A] dark:text-[#f8fafc] text-right">{data.taskTitle || "Task"}</span></div>
                <div className="flex justify-between gap-4"><span className="text-[#64748B]">Freelancer</span><span className="font-semibold text-[#0F172A] dark:text-[#f8fafc] text-right">{data.freelancerName}</span></div>
                <div className="flex justify-between gap-4"><span className="text-[#64748B]">Amount</span><span className="font-semibold text-[#0F172A] dark:text-[#f8fafc]">${data.amount}</span></div>
              </div>
            </div>
